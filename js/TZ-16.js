...
].map((film) =>  parseInt(film.rating) >= 92
    ? { rating: film.rating, cost: film.cost * 2, name: film.name }
    : film
);
/**
 ...
 */
 const arr = [
    ...
    { rating: "88%", cost: 37.6, name: "Неизведанное: Удача Дрейка" },
    { rating: "88%", cost: 36.5, name: "Топ Ган: Мэверик" },
  ].filter((film) =>  parseInt(film.rating) >= 90 

  
      //? { rating: film.rating, cost: film.cost, name: film.name }
      //: null
  );